...
/**
 * Information for removing diacritics when present.
 *
 * @type {*[]}
 */
const diacriticsMap = [
...
function getSlug(s) {
    ...
    for(let i = 0; i < diacriticsMap.length; i++) {
        result = result.replace(diacriticsMap[i].letters, diacriticsMap[i].base);
    }
    result = result.replace(/[^\w-_\s]+/g, "");
    result = result.replace(/\s+/g, '-');
    result = result.replace(/-+/g, '-'); // replace multiple '-' in a row
    return result;
}
...
module.exports.computePageProperties = computePageProperties;